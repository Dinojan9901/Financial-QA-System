...
\begin{titlepage}
  ...
  \vspace{0.9cm}
  \begin{tabular}{@{}ll@{}}
    \textbf{Course:}     & EC7203 Advanced Artificial Intelligence \\[0.45em]
    \textbf{Domain:}     & Banking \& Finance \\[0.45em]
    \textbf{Techniques:} & NLP, LLM, Prompt Engineering, Word Embeddings \\[0.45em]
  ...
  \vspace{0.4em}
  \begin{tabular}{@{}ll@{}}
    EG/2021/4487 & Dinojan V. \\[0.25em]
    EG/2021/4566 & Jackshan Venujan G.S. \\[0.25em]
    EG/2021/4825 & Tharshihan R. \\[0.25em]
    EG/2021/4805 & Senevirathna P.U.S. \\
  \end{tabular}

  \vspace{0.9em}
  {\small\textbf{Live Demo:}\\[0.3em]
  \url{https://financial-app-system-bcffpkdt88dxp8krfjg7pd.streamlit.app/}\par}
...
The core architecture is a \textbf{Retrieval-Augmented Generation (RAG)} pipeline combining three Advanced AI techniques: (1)~\textbf{Natural Language Processing}, text preprocessing, Word2Vec/TF-IDF baselines, and sentence-transformer embeddings; (2)~\textbf{Large Language Models}, a transformer LLM (Groq Llama~3.3 70B, free, or GPT-4o-mini) for grounded answer generation; and (3)~\textbf{Prompt Engineering}, systematic system prompt design, chain-of-thought reasoning, and few-shot in-context learning.

Experimental evaluation on \textbf{50 real question--answer pairs} from the public \texttt{financial-qa-10K} dataset (derived from genuine SEC 10-K filings) showed that MiniLM-L6-v2 sentence-transformer embeddings retrieved the correct source passage \textbf{first for every query} (\textbf{$\text{Hit}_1$ = 1.000}, \textbf{MRR = 1.000}), outperforming Word2Vec ($\text{Hit}_1$ = 0.640) and a strong TF-IDF baseline ($\text{Hit}_1$ = 0.940). The RAG pipeline recovered \textbf{84\% of gold-answer keywords} (KHR = 0.840), versus under $1\%$ for the ungrounded No-RAG baseline, empirically confirming that grounding is essential for accurate financial answers. The system is delivered as a deployable REST API (FastAPI) and interactive web app verified to run end-to-end on financial PDF inputs.
...
\multirow{2}{3.0cm}{\texttt{generation/}}
  ...
  & \texttt{qa\_chain.py}       & Groq/OpenAI LLM; LocalQAChain fallback \\
\midrule
...
Quantitative evaluation uses the publicly available \textbf{financial-qa-10K} dataset, hosted on the Hugging Face Hub (\url{https://huggingface.co/datasets/virattt/financial-qa-10K}). The dataset contains \textbf{7{,}000 question--answer pairs} automatically derived from real SEC 10-K annual-report filings of large public companies (e.g.\ NVIDIA, CVS, Eli Lilly, Etsy). Each record provides four fields used in our experiments:

\begin{itemize}[noitemsep]
  \item \texttt{question}, a natural-language financial question;
  \item \texttt{answer}, the ground-truth answer string;
  \item \texttt{context}, the exact passage from the 10-K that supports the answer (the gold retrieval target);
  \item \texttt{ticker} / \texttt{filing}, provenance (company and source filing, e.g.\ \texttt{NVDA 2023\_10K}).
\end{itemize}

\textbf{Sampling methodology.} For reproducibility we draw a fixed random sample of \textbf{50 question--answer pairs} (seed~=~42) and cache it locally as JSON, so every run is deterministic and offline after the first download. Within each retrieval experiment the 50 gold \texttt{context} passages form a shared candidate pool: for any given question exactly one passage is the correct target and the remaining 49 act as distractors. Answer keywords for the Keyword-Hit-Rate metric are derived automatically from each gold \texttt{answer} (content words and numeric tokens, stop words removed). The loader and methodology are implemented in \texttt{evaluation/dataset\_loader.py}.

For the \textit{intrinsic} embedding-quality experiment (Experiment~3, Separability Gap) we additionally use a small curated set of semantically similar and dissimilar financial sentence pairs as controlled probes, in the style of standard word/sentence-similarity benchmarks.
...
\begin{align}
  \text{Hit}_K &= \frac{1}{|Q|}\sum_{i=1}^{|Q|}
      \mathbb{1}\!\left[\text{rank}_i \le K\right]
  \\[4pt]
...
where $\text{rank}_i$ is the position of the gold source passage for query~$i$ in the ranked candidate list. $\text{Hit}_1$ (equivalently Precision@1 for a single relevant item) is the fraction of queries whose exact source passage is retrieved first, and $\text{Hit}_3$ within the top three. Keyword Hit Rate measures answer quality: the fraction of gold-answer keywords present in the generated answer.
...
Table~\ref{tab:baseline} reports gold source-passage retrieval over the 50 real SEC 10-K question--answer pairs. For each question the method ranks all 50 candidate passages; we measure whether the question's exact source passage is retrieved at rank~1 ($\text{Hit}_1$) or within the top three ($\text{Hit}_3$), its mean reciprocal rank (MRR), and the top-3 keyword overlap with the gold answer ($P_3$).

\begin{table}[htbp]
\centering
\caption{Retrieval quality on 50 real SEC 10-K Q\&A pairs: TF-IDF vs.\ Word2Vec vs.\ SentenceTransformer}
\label{tab:baseline}
\setlength{\tabcolsep}{8pt}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{L{4.6cm} C{1.6cm} C{1.6cm} C{1.5cm} C{1.5cm} C{2.0cm}}
\toprule
\textbf{Method} & $\boldsymbol{\text{Hit}_1}$ & $\boldsymbol{\text{Hit}_3}$ & \textbf{MRR} & $\boldsymbol{P_3}$ & \textbf{Q.\ Time} \\
\midrule
TF-IDF (sparse, n-gram 1--2)              & 0.940 & 1.000 & 0.963 & \textbf{0.620} & 10.2\,ms \\
Word2Vec (skip-gram, $d$=100)             & 0.640 & 0.760 & 0.717 & 0.447 & 4.3\,ms \\
\textbf{SentenceTransformer (MiniLM-L6-v2)} & \textbf{1.000} & \textbf{1.000} & \textbf{1.000} & 0.580 & 139.3\,ms \\
\bottomrule
\end{tabular}
\end{table}

On real filings the SentenceTransformer retrieves the correct source passage \textbf{first for every query} ($\text{Hit}_1 = 1.000$, MRR $= 1.000$). TF-IDF is a surprisingly strong baseline ($\text{Hit}_1 = 0.940$), because financial questions frequently reuse the exact terminology of their source passage, a setting that favours lexical matching; it even leads on the keyword-overlap metric $P_3$. Word2Vec is weakest ($\text{Hit}_1 = 0.640$): trained only on the small in-corpus vocabulary, it generalises poorly to paraphrased queries. The semantic model's advantage is robustness, it never misses, whereas the lexical baseline fails on the $6\%$ of queries that paraphrase their source. The trade-off is latency ($139$\,ms vs.\ $4$--$10$\,ms), still well within interactive limits.

\subsubsection{Experiment 2: RAG vs.\ No-RAG Evaluation}

Table~\ref{tab:ragnorag} compares three answer strategies across all 50 real questions, measured by Keyword Hit Rate (KHR), the fraction of gold-answer keywords present in the generated answer.

\begin{table}[htbp]
\centering
\caption{RAG vs.\ No-RAG vs.\ Random-Context on 50 real Q\&A pairs (Keyword Hit Rate)}
\label{tab:ragnorag}
\setlength{\tabcolsep}{10pt}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{L{7.0cm} C{4.0cm}}
\toprule
\textbf{Strategy} & \textbf{Keyword Hit Rate} \\
\midrule
No-RAG (LLM only, no document context)      & 0.008 \\
Random Context (irrelevant chunks)          & 0.133 \\
\textbf{RAG (retrieved relevant chunks)}    & \textbf{0.840} \\
\bottomrule
\end{tabular}
\end{table}

The contrast is decisive: RAG recovers $84\%$ of gold-answer keywords, versus essentially zero ($0.8\%$) for the ungrounded No-RAG baseline, which cannot access filing-specific figures. The Random-Context ablation ($13.3\%$) confirms that context alone is insufficient, \textit{relevant} context is what matters. (The faithfulness and hallucination heuristics used in the smaller pilot become degenerate on this larger, more diverse set when run in offline simulation mode, so we report the robust KHR signal here; with a live LLM API key the harness additionally records per-answer faithfulness.)

\subsubsection{Experiment 3: Intrinsic Embedding Benchmark}

Whereas Experiment~1 measures end-to-end retrieval on the real dataset, Experiment~3 isolates \textit{intrinsic} embedding quality on a curated set of controlled similar/dissimilar financial sentence pairs. Table~\ref{tab:benchmark} reports the Separability Gap together with retrieval metrics on this probe set.

\begin{table}[htbp]
\centering
\caption{Intrinsic embedding benchmark on curated probe pairs: separability gap, Precision, MRR, NDCG, and latency}
\label{tab:benchmark}
\setlength{\tabcolsep}{8pt}
\renewcommand{\arraystretch}{1.2}
...
The RAG system recovers $84\%$ of gold-answer keywords across the 50 real questions, versus under $1\%$ for the ungrounded baseline. The key mechanism is the system prompt constraint ``ONLY use information from the provided document excerpts,'' which forces the model to ground its answers in retrieved content rather than LLM-memorised approximations.
...
\begin{itemize}[leftmargin=*, noitemsep]
  \item \textbf{Sample size:} We evaluate on a fixed 50-pair sample of the 7{,}000-pair dataset for fast, reproducible runs; scaling to the full set (and to multiple sampled folds) would tighten confidence intervals.
  \item \textbf{Approximate faithfulness:} Keyword Hit Rate and the faithfulness heuristic approximate answer quality via lexical overlap. Production systems should use an NLI (Natural Language Inference) model to score entailment between answer and retrieved context.
  \item \textbf{Local simulation mode:} Without an OpenAI API key, the RAG vs.\ No-RAG experiment uses simulated rather than live GPT responses; the keyword-hit-rate contrast remains valid, but faithfulness/hallucination heuristics are less informative in this mode.
\end{itemize}
...
The Random Context ablation (KHR = 0.133) demonstrates that providing any context is insufficient, the \textit{right} context is necessary. This underscores that the retrieval component is the critical bottleneck in the RAG pipeline.

\textbf{Grounding prevents fabrication.}
The structured system prompt with explicit ``do not speculate'' rules, combined with relevant retrieved context, keeps answers anchored to the source filing. The ungrounded No-RAG baseline, lacking access to filing-specific figures, recovered almost none of the expected answer content (KHR = 0.008).
...
\begin{itemize}[leftmargin=*, noitemsep]
  ...
  \item \textbf{Context window limits:} Modern LLMs (GPT-4o-mini, Llama~3.3 70B) support 128K-token contexts, sufficient for 5 retrieved chunks, but retrieving 20+ chunks would require context compression.
...
\begin{enumerate}[leftmargin=*, noitemsep]
  ...
  \item \textbf{LLM:} Decoder-only transformer generation via the OpenAI-compatible Chat Completions API, Groq (free, Llama~3.3 70B) by default or OpenAI GPT-4o-mini, for grounded answer generation.
...
Experimental results on 50 real SEC 10-K question--answer pairs confirm the superiority of the proposed system: sentence-transformer embeddings achieve $\text{Hit}_1$ = 1.000 and MRR = 1.000, outperforming both the Word2Vec and TF-IDF baselines. The RAG pipeline recovers $84\%$ of gold-answer keywords, versus under $1\%$ for the ungrounded No-RAG baseline.
...
\item Bhutani, V. (2024). \textit{financial-qa-10K} [Data set]. Hugging Face. \url{https://huggingface.co/datasets/virattt/financial-qa-10K}

\item U.S. Securities and Exchange Commission. (n.d.). \textit{EDGAR full-text search system}. \url{https://www.sec.gov/cgi-bin/browse-edgar}
...
The figures below visualise the evaluation results from Tables~\ref{tab:baseline},~\ref{tab:ragnorag}, and~\ref{tab:benchmark}. They are produced by \texttt{generate\_appendix\_charts.py}; the full machine-readable results are saved as timestamped JSON in \texttt{evaluation/results/} via \texttt{python run\_evaluation.py \text{-{}-}save}.

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../figure_B1_retrieval_comparison.png}
\caption{Retrieval quality on 50 real SEC 10-K Q\&A pairs, Hit@1, Hit@3, and MRR for TF-IDF, Word2Vec, and the proposed SentenceTransformer (MiniLM-L6-v2).}
\label{fig:appendix-b1}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../figure_B2_rag_vs_norag.png}
\caption{RAG vs.\ No-RAG vs.\ Random-Context, Keyword Hit Rate across the 50 questions. RAG recovers $84\%$ of gold-answer keywords versus under $1\%$ without grounding.}
\label{fig:appendix-b2}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../figure_B3_embedding_benchmark.png}
\caption{Intrinsic embedding benchmark, separability and retrieval metrics for Word2Vec, TF-IDF, and MiniLM-L6-v2 on the curated probe set.}
\label{fig:appendix-b3}
\end{figure}
...
\begin{tabular}{L{2.6cm} L{3.4cm} L{6.4cm}}
\toprule
\textbf{Reg. No.} & \textbf{Name} & \textbf{Contribution} \\
\midrule
