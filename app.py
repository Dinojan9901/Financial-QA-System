...
with st.sidebar:
    ...
    # Embeddings always run on the free local MiniLM model.
    os.environ["USE_LOCAL_MODELS"] = "true"

    from config import resolve_llm_provider
    _active, _, _, _ = resolve_llm_provider()

    if _active in ("groq", "openai"):
        # A key is already configured (deployment secret / .env) — no need to
        # ask visitors for one. Just show the active provider.
        _label = "Groq (Llama 3.3 70B)" if _active == "groq" else "OpenAI (GPT-4o-mini)"
        st.success(f"🟢 Answers generated by {_label}")
    else:
        # No key configured — let a local user optionally add a free Groq key.
        groq_key = st.text_input(
            "Groq API Key — free (optional)",
            type="password",
            placeholder="gsk_... (free at console.groq.com/keys)",
            help="Free LLM generation (Llama 3.3 70B). Without a key the app "
                 "returns the most relevant retrieved sentence.",
        )
        if groq_key:
            os.environ["GROQ_API_KEY"] = groq_key
            os.environ["LLM_PROVIDER"] = "groq"
            st.success("🟢 Answers generated by Groq (Llama 3.3 70B)")
        else:
            os.environ["LLM_PROVIDER"] = "auto"
            st.info("🔵 Local mode — returns the most relevant sentence")
